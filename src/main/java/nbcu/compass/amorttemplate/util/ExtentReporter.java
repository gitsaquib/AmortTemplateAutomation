...
public class ExtentReporter {
	...
	private static void setInterruptedTestStatus(ITestResult iTestResult, ExtentTest extentTest) {
		if(!(extentTest.getRunStatus().equals(LogStatus.UNKNOWN) || extentTest.getRunStatus().equals(LogStatus.PASS))) {
			return;
		}
	...
	public static void setTestStatusAsSkip(ITestResult result) {
		try {
			...
			test.getTest().getLogList().forEach(log -> {
				if (log.getLogStatus() == LogStatus.ERROR || log.getLogStatus() == LogStatus.FAIL || log.getLogStatus() == LogStatus.FATAL) {
